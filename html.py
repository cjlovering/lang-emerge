...
class HTML():
    def __init__(self, cols):
        self.template = '<html><head><style>'+\
                        ...
                        +'table#t01 th{background-color:blackcolor:white}'+\
    ...
    def addRow(self, *entries):
        ...
        if len(entries) < self.numCols: # add 'null'
            for ii in range(self.numCols - len(entries)):
                entries.append('NULL')
    ...
    def savePage(self, filePath):
        ...
        pageContent = pageContent.replace('\t', '&nbsp'*10)
    ...
    def attentionRow(self, attWt):
        numQues = attWt.size
        titles = ['Cap']
        titles.extend(['%02d' % ii for ii in range(1, numQues)])
        string = '</br>'

        maxAtt = np.max(attWt)
        for ii in range(0, numQues):

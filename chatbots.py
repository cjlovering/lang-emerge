...
class ChatBot(nn.Module):
    ...
    def listen(self, inputToken, imgEmbed = None):
        ...
        self.hState, self.cState = self.rnn(tokenEmbeds, (self.hState, self.cState))
...
class Answerer(ChatBot):
    ...
    def embedImage(self, batch):
        ...
        features = embeds.view(-1,1,60).squeeze(1)
        return features
...
class Questioner(ChatBot):
    ...
    def guessAttribute(self, inputEmbeds):
        # compute softmax and choose a token
        self.hState, self.cState = self.predictRNN(
            inputEmbeds, (self.hState, self.cState))
    ...
    def embedTask(self, tasks): 
        return self.inNet(tasks + self.taskOffset)
